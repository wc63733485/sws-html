    ...
    $.ajax({
        ...
        success: function (r)   {
            ...
            for (var i = 0; i < r.data.length; i++) {
                if (r.data[i].parentId == 0) {
                    ...
                    if (r.data[i].menuType == "C") {
                        var ahref = "<a  href='" + r.data[i].url + "' class='menuItem' >" + icon + "<span class='nav-label'>" + menuName + "</span></a>";

                        html = html + "<li>" + ahref + "</li>"
                    } else if (r.data[i].menuType == "M") {
                        var menuType = "<ul class='nav nav-second-level collapse'>"
                        html = html + "<li><a href='#' >" + icon + "<span class='nav-label'>" + menuName + "</span><span class='fa arrow'></span></a>" + menuType;
                        for (var z = 0; z < r.data.length; z++) {
                            if (r.data[z].parentId == r.data[i].menuId) {
                                var icon = "<i class='" + r.data[z].icon + "'></i> ";
                                var menuName = r.data[z].menuName;
                                if (r.data[z].menuType == "C") {
                                    var ahref = "<a href='" + r.data[z].url + "' class='menuItem' >" + icon + "<span class='nav-label'>" + menuName + "</span></a>";

                                    html = html + "<li>" + ahref + "</li>";
                                } else if (r.data[z].menuType == "M") {
                                    var menuType = "<ul class='nav nav-third-level'>"
                                    html = html + "<li><a href='#' >" + icon + "<span class='nav-label'>" + menuName + "</span><span class='fa arrow'></span></a>" + menuType;
                                    for (var c = 0; c < r.data.length; c++) {
                                        if (r.data[c].parentId == r.data[z].menuId) {
                                            var icon = "<i class='" + r.data[c].icon + "'></i> ";
                                            var menuName = r.data[c].menuName;
                                            if (r.data[c].menuType == "C") {
                                                var ahref = "<a href='" + r.data[c].url + "' class='menuItem' >" + icon + "<span class='nav-label'>" + menuName + "</span></a>";

                                                html = html + "<li>" + ahref + "</li>";
                                            } else if (r.data[c].menuType == "M") {
                                                var menuType = "<ul class='nav nav-fourth-level'>"
                                                html = html + "<li><a href='#' >" + icon + "<span class='nav-label'>" + menuName + "</span><span class='fa arrow'></span></a>" + menuType;
                                                html = html + "</ul></li>";
                                            }
                                        }
            ...
            function menuItem() {
                ...
                // 选项卡菜单不存在
                if (flag) {
                    var str = '<a href="javascript:;" class="active menuTab" data-id="' + dataUrl + '">' + menuName + ' <i class="layui-icon layui-unselect layui-icon-close"></i></a>';
            ...
            function setIframeUrl(href) {
                if ($.common.equals("history", mode)) {
                    localStorage.set('publicPath', href);
                } else {
