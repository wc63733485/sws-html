...
function getCityBuy(val){
	...
	newStr.push("<li onclick=\"getProvinceBuy()\" style=\"background-color:orange;\">"+province[val].NAME+"</li>");
...
function getAreaBuy(val,val1){
	...
	var newStr=new Array();
	newStr.push("<div class=\"dqld_div\"><ul>");
	newStr.push("<li onclick=\"getProvinceBuy()\" style=\"background-color:orange;\">"+province[val1].NAME+"</li>");
	...
	if(asize==0){
		...
		if(province[val1].NAME != city[val].NAME){
			$("#shengshi").attr({"SS":province[val1].NAME,"SQ":city[val].NAME,"XS":""});
			$("#shengshi").val(allarea);
		}else{
			$("#shengshi").attr({"SS":"","SQ":city[val].NAME,"XS":""});
			$("#shengshi").val(province[val1].NAME);
		}
		$("body .dqld_div").remove();
		var value = $('.address_input1').val();
		sessionStorage.setItem('xxrz_city',value);
		window.history.go(-1);
	}
	else{
		$("body .dqld_div").remove();
...
function getallArea(val,val1,val2){
	...
	if(province[val].NAME != city[val1].NAME){
		$("#shengshi").attr({"SS":province[val].NAME,"SQ":city[val1].NAME,"XS":area[val2].NAME});
		$("#shengshi").val(allarea);
	}else{
		$("#shengshi").attr({"SS":province[val].NAME,"SQ":"","XS":area[val2].NAME});
		$("#shengshi").val(city[val1].NAME+area[val2].NAME);
	}
